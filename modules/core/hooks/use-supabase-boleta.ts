...
interface UseSupabaseBoletaReturn {
  loading: boolean;
  error: string | null;
  insertBoleta: (data: Omit<BoletaInsert, "user_id">) => Promise<BoletaRow | null>;
...
export const useSupabaseBoleta = (): UseSupabaseBoletaReturn => {
  ...
  const insertBoleta = useCallback(async (data: Omit<BoletaInsert, "user_id">) => {
    setLoading(true);
    setError(null);
  ...
  const updateBoleta = useCallback(async (id: string, data: Partial<BoletaRow>) => {
    setLoading(true);
    setError(null);
